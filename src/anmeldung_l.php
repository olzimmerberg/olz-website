...
// =============================================================================
// Geplantes Feature, um sich online für Anlässe anmelden zu können.
// =============================================================================
...
} elseif (isset($id_event) and is_ganzzahl($id_event)) {
    $_SESSION[$db_table."id_event_"] = $id_event;
} else {
...
if ($id_event == "") {
    ...
    if ($row['datum_end'] > $heute) {
        ...
        $link = $row['link'];
        if ($datum != $datum_end) {
            $datum = olz_date("t.m.", $datum)." - ".olz_date("t.m.jjjj", $datum_end);
        } else {
            $datum = olz_date("t.m.jjjj", $datum);
        }
        $link = str_replace(["</div><div", "<div", "</div"], ["</span> | <span", "<span", "</span"], $link);
    ...
    if ((($db_edit == "0") or ($do == "vorschau")) and ($id_event != "")) {
        if ($do != "vorschau") {
            ...
            if (($db_edit == "0") and ($datum_anm > $heute)) {
                echo "<div class='buttonbar'>".olz_buttons("button".$db_table, [["Neue Anmeldung", "0"], ["Anmeldung bearbeiten", "1"]], "")."</div>";
            } elseif ($db_edit == "0") {
            ...
            echo "</table>";
        } else {// VORSCHAU ANMELDUNG
            echo "<table class='liste'>";
            $felder_tmp = [["datum", "Datum"], ["name", "Name, Vorname"], ["email", "Email-Adresse"], ["anzahl", "Anzahl Teilnehmer"]];
            $sql = "SELECT * from anm_felder WHERE (event_id='{$id_event}') ORDER BY position ASC";
            $result = mysql_query($sql);
            while ($row = mysql_fetch_array($result)) {
                array_push($felder_tmp, ["feld".$row['position'], $row['label'], $row['typ']]);
            }
