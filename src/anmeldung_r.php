...
if ($zugriff) $sql = "SELECT * FROM $db_table WHERE (datum_anmeldung>'0000-00-00') ORDER BY datum ASC";
else $sql = "SELECT * FROM $db_table WHERE (datum_anmeldung>'0000-00-00') AND (datum >= '".olz_date("jjjj-mm-tt","")."') ORDER BY datum ASC";
...
while ($row = mysql_fetch_array($result))
	...
	if ($zugriff) $edit_admin = "<a href='index.php?page=3&amp;id=$id&amp;button$db_table=start' class='linkedit'>&nbsp;</a>";
	else $edit_admin = "";
...
$result = mysql_query($sql);
while ($row = mysql_fetch_array($result))
	{array_push($event,array($row['titel'],$row['go2ol'],strtotime($row['datum'])));
	}
foreach ($event as $thisevent)
	...
	curl_close($ch);
	if (trim($file) == "")
		{//echo "Go2ol-Daten können nicht abgefragt werden.";
		}
	else
