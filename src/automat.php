...
function automat_solv_view() {
    ...
    if (isset($_GET['merge']) && isset($_GET['into'])) {
    ...
    for ($i=0; $i<$res_p->num_rows; $i++) {
        ...
        if (2<$dist_residence) $dist_residence = 2;
        $dist = ($dist_name+$dist_birthyear+$dist_residence);
        if ($dist<10) {
            if (!isset($dists[$dist])) $dists[$dist] = array();
            $dists[$dist][] = array($row_p['name1'], $row_p['birthyear1'], $row_p['residence1'], $row_p['id1'], $row_p['name2'], $row_p['birthyear2'], $row_p['residence2'], $row_p['id2']);
        }
        $row_p = $res_p->fetch_assoc();
    }
    echo "<h2>&Auml;hnliche Namen</h2><table>";
    for ($dist=1; $dist<10; $dist++) {
        if (!isset($dists[$dist])) continue;
...
function automat_solv_update($incr=true) {
    ...
    while (true) {
        ...
        for ($i=0; $i<$cnt_data; $i++) {
            ...
            } else if ($last_modification[$uid]!=$data[$i]['last_modification']) {
                $sql_tmp = array();
                foreach ($solv_events_fields as $db_key=>$header_key) {
        ...
        for ($i=0; $i<count($resp["ResultLists"]); $i++) {
            ...
            if (!isset($rank_data[$res['UniqueID']])) {
                $rank_data[$res['UniqueID']] = array();
            }
            $rank_data[$res['UniqueID']][] = $res;
        }

        if (count($data)<3) break;
        if (date('Y')+3<$year) break;
        ...
        for ($result_index=0; $result_index<count($rank_data[$solv_uid]); $result_index++) {
            ...
            for ($j=0; $j<$num; $j++) {
                if (substr($resp,$j,6)=="</pre>") {
                    ...
                    if (!$r) echo "ERROR: No Match for Class definition<br>";
                    if ($matches[3]!=urlencode($matches[5])) echo "ERROR: Inconsistent Match for Class definition<br>";
                    if ($r) {
                        $last_pre = $j+6;
                        $inpre = false;
                        $pres[$matches[5]] = $pre;
            ...
            for ($j=0; $j<$num; $j++) {
                ...
                while (true) {
                    ...
                    if ($member) {
                        ...
                        if ($res_r->num_rows==0) $db->query("INSERT INTO solv_results (person, event, class, result) VALUES ('".intval($person)."', '".intval($row_e['solv_uid'])."', '".DBEsc($keys[$j])."', '".$cur_zeit."')");
                    }
                }
            }
        }
...
$modules = array(
    ...
    'solv'=>array(
        ...
        'update'=>function () {
            $incremental = true;
            if ($_GET['incremental']=='no') $incremental = false;
            automat_solv_update($incremental);
        },
    ),
);
