...
// MENÜ
if ($db_edit=="0")
    {echo "<div class='buttonbar'>".olz_buttons("button".$db_table,array(array("Neuer Eintrag","0"),array("Eintrag bearbeiten","1")),"")."</div>";
    if ($zugriff)
...
if (($db_edit=="0") OR ($do=="vorschau"))
    ...
    elseif ($zugriff)
        {if ($_SESSION[$db_table."monat_"]=="alle") $sql = "WHERE (YEAR(datum)='".$jahr."')";
    ...
    if ($do=="vorschau") $sql = "WHERE (id = ".$_SESSION[$db_table.'id'].")";
    ...
    while($row = mysqli_fetch_array($result))
        ...
        for ($i=0; $i<count($matches[0]); $i++) {
            $class = "linkext";
            if (strpos($matches[0][$i],"olzimmerberg.ch")!==false) $class = "linkint";
            elseif (strpos($matches[0][$i],"maps.google")!==false) $class = "linkmap";
            elseif (strpos($matches[0][$i],"map.search.ch")!==false) $class = "linkmap";
            elseif (strpos($matches[0][$i],"sbb.ch")!==false) $class = "linkoev";
            elseif (strpos($matches[0][$i],".pdf")===strlen($matches[0][$i])-4) $class = "linkpdf";
            elseif (strpos($matches[0][$i],".png")===strlen($matches[0][$i])-4) $class = "linkimg";
            elseif (strpos($matches[0][$i],".jpg")===strlen($matches[0][$i])-4) $class = "linkimg";
            elseif (strpos($matches[0][$i],".jpeg")===strlen($matches[0][$i])-5) $class = "linkimg";
            elseif (strpos($matches[0][$i],".gif")===strlen($matches[0][$i])-4) $class = "linkimg";
            elseif($row['allowHTML']) $class = 'linkext';
        ...
        if (($on_off==0) AND ($do!="vorschau")) $class = " class='error'";
        else $class = "";
        ...
        if($name>"") echo $titel."</b><p>".olz_mask_email($email,$name,$titel)."| ".$eintrag."</p></td></tr>\n";
        else echo olz_mask_email($email,$titel,"Forumeintrag OL Zimmerberg")."</b><p>".$eintrag."</p></td></tr>\n";
        }
    echo "</table>";
    }
?>