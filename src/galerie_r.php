...
if (isset($_GET["jahr"])) $_SESSION[$db_table."jahr_"] = $_GET["jahr"];
...
<div id='Accordion".$tmp_jahr."Content' class='AccordionContent'".($_SESSION[$db_table.'jahr_']==$tmp_jahr?" style='height:auto;'":"")."><div id='Accordion".$tmp_jahr."Content_' class='AccordionContent_'>";
        ...
        while ($row = mysqli_fetch_array($result))
            ...
            else $linkclass = "linkimg" ;

            if ($zugriff AND ($do != 'vorschau')) $edit_admin = "<li style='opacity:".($on_off?"1":"0.5").";'><a href='index.php?id=".$id_tmp."&amp;button$db_table=start' class='linkedit'>&nbsp;</a>";
            else $edit_admin = "<li>";

            if ($id == $id_tmp) echo "$edit_admin<span class='linkblack' style='font-weight:bold;'>".date("j",strtotime($datum)).". ".ucfirst($monate[date("n",strtotime($datum))-1]).": " . $titel . " (" . $groesse . ")</span></li>";
            else echo "<li>$edit_admin<a href='index.php?page=$page&amp;id=" . $id_tmp . "".(isset($_GET["archiv"])?"&amp;archiv":"")."' class='$linkclass' id='galerie_r_a_".$id_tmp."'>".date("j",strtotime($datum)).". ".ucfirst($monate[date("n",strtotime($datum))-1]).": " . $titel . " (" . $groesse . ")</a></li>";
            if ($do=="edit")
