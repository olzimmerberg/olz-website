...
function solvdataforyear($year) {
    ...
    for ($i=0; $i<count($solv_termine); $i++) {
        ...
        if ($tmp["datum"]) {
            ...
            if ($num==0) $db->query("INSERT INTO solv_events (solv_uid) VALUES ('".intval($tmp[0])."')", $db);
...
function go2oldata() {
    ...
    for ($i=0; $i<count($matches["link"]); $i++) {
        ...
        if (0<$ohne) $tmp["meldeschluss"][] = $ohne;
        $mit = strtotime($matches["meldeschluss_mit"][$i]);
        if (0<$mit) $tmp["meldeschluss"][] = $mit;
        $go2ol_termine[] = $tmp;
        if (0<$tmp["solv_uid"]) {
            $result = $db->query("SELECT solv_uid FROM termine_go2ol WHERE solv_uid='".intval($tmp["solv_uid"])."'", $db);
            $num = mysqli_num_rows($result);
            if ($num==0) $db->query("INSERT INTO termine_go2ol (solv_uid) VALUES ('".intval($tmp["solv_uid"])."')", $db);
