...
function solvdataforuid($uid) {
    ...
    $solv_lauf = array("datum"=>"","meldeschluss"=>"","region"=>"","abk"=>"","links"=>"","name"=>"","verein"=>"","karte"=>"","map"=>"","rangliste"=>"","startliste"=>"");
    $solv_lauf["links"] = array();
    ...
    if (trim($file) == "") {
        ...
        $numi = strlen($file);
        $keys = array("Datum:"=>"datum","Region:"=>"region","Club:"=>"verein","Karte:"=>"karte","Meldeschluss:"=>"meldeschluss","Koordinaten:"=>"map");
        $checkpoint = "start";
        $key = "";
        $linktmp = array();
        for ($i=0; $i<$numi; $i++) {
            ...
            if ($checkpoint=="valtrcont" && check($file,$i,"<td")) {
                ...
                } else if (!is_bool(strpos($key,"Rangliste"))) {
                    $solv_lauf["rangliste"] = $key;
                } else if (!is_bool(strpos($key,"Startliste"))) {
            ...
            if ($checkpoint=="alink" && check($file,$i,"href=\"")) {
                ...
                $linktmp = array("href"=>$link_tmp,"text"=>"");
                $checkpoint = "linkhref";
            }
...
function parse_map($str) {
    ...
    for ($i=0; $i<count($coords); $i++) {
        if (400000<$coords[$i] AND $coords[$i]<900000) {
            $return["x"] = $coords[$i];
        } else if (50000<$coords[$i] AND $coords[$i]<400000) {
            $return["y"] = $coords[$i];
        }
    }
    return $return;
...
function parse_links($str) {
    ...
    for ($i=0; $i<strlen($str); $i++) {
        if ($status=="div" && check($str,$i,"</div")) {
            array_push($links,array("type"=>$linktype,"href"=>$linkhref,"text"=>$linkname));
            $linktype = "";
            $linkhref = "";
            $linkname = "";
            $status = "text";
        ...
        if (substr($str,$i,7)=="linkint") {$linktype = "linkint";}
        if (substr($str,$i,7)=="linkext") {$linktype = "linkext";}
        if (substr($str,$i,7)=="linkpdf") {$linktype = "linkpdf";}
        if (substr($str,$i,8)=="linkmail") {$linktype = "linkmail";}
        if (substr($str,$i,7)=="linkmap") {$linktype = "linkmap";}
        if (substr($str,$i,7)=="linkoev") {$linktype = "linkoev";}
        if (substr($str,$i,7)=="linkimg") {$linktype = "linkimg";}
        if (substr($str,$i,9)=="linkmovie") {$linktype = "linkmovie";}
            //echo $linktype."<br>";
            $status = "classgot";
        }
...
function geturlcontent($url) {
    $allowedcharsets = array("ISO-8859-1","ISO-8859-2","ISO-8859-3","WINDOWS-1250","WINDOWS-1252","UTF-8");
    // ---
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    $file = curl_exec($ch);
