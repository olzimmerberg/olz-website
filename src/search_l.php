...
$length_a = 40;
$length_b = 40;
$search_key = trim(str_replace(array(",",".",";","   ","  "),array(" "," "," "," "," "),$search_key));
...
for ($n=0;$n<3;$n++)
    ...
    if ($n>0) $or = " AND ";
    if ($search_key>"") $sql1 .= $or."((titel LIKE '%$search_key%') OR (text LIKE '%$search_key%'))";
    if ($search_key>"") $sql2 .= $or."((name LIKE '%$search_key%') OR (eintrag LIKE '%$search_key%'))";
    if ($search_key>"") $sql3 .= $or."(titel LIKE '%$search_key%')";
    if ($search_key>"") $search .= $or."$search_key";
    }
...
if ($text!='') echo "<table class='liste'>".$text."</table>";
...
for ($m=0;$m<3;$m++)
    ...
    if (strlen($text)<($length_a+$length_b)) $suffix = "";
