...
if (isset($id) AND is_ganzzahl($id) AND ($_SESSION['edit']['db_table']==$db_table)) $_SESSION[$db_table."id_"] = $id;
...
if ($zugriff)
    {if ($db_edit=="1") $sql = "SELECT * FROM $db_table WHERE (id='".$_SESSION[$db_table."id_"]."') ORDER BY position ASC";
    else $sql = "SELECT * FROM $db_table ORDER BY  position ASC";
    }
...
if (($db_edit=="0") OR ($do=="vorschau"))
    ...
    while($row = mysqli_fetch_array($result))
        ...
        $name = $row['name'];
        $url = $row['url'];
        if ($zugriff AND ($do != 'vorschau')) $edit_admin = "<a href='index.php?id=$id_tmp&amp;$button_name=up' style='margin-right:4px;'><img src='icns/up.gif' class='noborder'></a><a href='index.php?id=$id_tmp&amp;$button_name=down' style='margin-right:4px;'><img src='icns/down.gif' class='noborder'></a><a href='index.php?id=$id_tmp&$button_name=start' class='linkedit'>&nbsp;</a>";
        else $edit_admin = "";
        ...
        if ($name=="dummy")
            {if ($zugriff) echo $edit_admin."-----Trennlinie-----";
            else echo "<br>";
            }
        elseif ($db_edit =="0")
