...
if ($aktuell_typ!='aktuell') $sql = "SELECT * from aktuell WHERE typ='$aktuell_typ' ORDER BY datum DESC";
else $sql = "
...
while ($row = $result->fetch_assoc())
    ...
    if ($thistype=="blog") // Tabelle 'blog'
       ...
       /* preg_match_all("/<datei([0-9]+)(\s+text=(\"|\')([^\"\']+)(\"|\'))?([^>]*)>/i", $text, $matches);
    ...
    else if ($thistype=="galerie") // Tabelle 'galerie'
        {$pfad = $row['id'];
        $typ = $row['f3'];
        $link = "?page=4&amp;id=".$id;
        $icon = "icns/foto.png";
        ...
        if ($aktuell_typ!='aktuell') $text = $row['textlang'];
        if(strip_tags($textlang)>'') $text .= " [...]";
