...
<?php
$db_table = "termine";
$ter_filter = array (array("alle","Alle Termine"),array("training","Training"),array("ol","Wettkämpfe"),array("resultat","Resultate"),array("club","Vereinsanlässe"));
...
if (isset($jahr) AND in_array($jahr,$jahre)) $_SESSION[$db_table."jahr_"] = $jahr;
else $jahr = $_SESSION[$db_table."jahr_"];
if ($jahr=="") $_SESSION[$db_table.'jahr_'] = olz_date("jjjj","");
if (isset($monat) AND in_array($monat,$monate)) $_SESSION[$db_table."monat_"] = $monat;
else $monat = $_SESSION[$db_table."monat_"];
if ($monat=="") $_SESSION[$db_table.'monat_'] = "alle";
if (isset($filter) AND in_array($filter,array('alle','training','ol','club','resultat'))) $_SESSION['termin_filter'] = $filter;
elseif (!isset($_SESSION['termin_filter'])) $_SESSION['termin_filter'] = "alle";
...
if ($zugriff AND $db_edit=="0")
    {echo "<div class='buttonbar'>".olz_buttons("button".$db_table,array(array("Neuer Eintrag","0")),"")." <span class='linkint'><a href='?page=15'>Termine-Tools</a></span></div>";
...
// Filter-Dropdown nach Typ
    ...
    foreach ($ter_filter as $this_filter)
        {if ($_SESSION['termin_filter']==$this_filter[0]) $selected = " style='text-decoration:underline;'";
...
// Vergangene ein-/ausblenden
    ...
    elseif (!isset($show)) $show = '0';
    $_SESSION['show'] = $show;
    if ($show == "1") $checked = " checked";
    ...
    foreach($termin_ as $tmp_termin){
        if($tmp_termin==$_SESSION[$db_table."datum"]){ // UPDATE Startdatum
            ...
            foreach ($db_felder as $tmp_feld)
                {if($tmp_feld[0]=='datum') array_push($sql_tmp,$tmp_feld[0]." = '".date("Y-m-d",$tmp_termin)."'");
                elseif($tmp_feld[0]=='datum_end') array_push($sql_tmp,$tmp_feld[0]." = '0000-00-00'");
...
if (($db_edit=="0") OR ($do=="vorschau"))
    {// ADMIN Mysql-Abfrage definieren
    if ($zugriff)
        {if ($monat=="alle") $sql = "WHERE YEAR(datum)= '$jahr'";
        else $sql = "WHERE (MONTH(datum)='".(array_search($monat,$monate)+1)."') AND (YEAR(datum)='$jahr')";
        }
    ...
    if ($_SESSION['termin_filter'] == "alle") $sql = $sql." ORDER BY datum ASC";
    //elseif ($_SESSION['termin_filter'] == "resultat") $sql = $sql." AND ((link LIKE '%resultat%') OR (link LIKE '%rangliste%')) ORDER BY datum DESC";
    elseif ($_SESSION['termin_filter'] == "resultat") $sql = $sql." AND ((typ LIKE '%ol%') AND (datum <= '$heute')) ORDER BY datum DESC";
    else $sql = $sql." AND (typ LIKE '%".$_SESSION['termin_filter']."%') ORDER BY datum ASC";
    if ($do=="vorschau") $sql = "WHERE (id ='$id')"; // Proforma-Abfrage
    ...
    while ($row = mysqli_fetch_array($result))
        ...
        } else if ($row_solv["entryportal"] == 1 AND $datum >= $heute) {
        ...
        if ($newsletter) $icn_newsletter = "<img src='icns/newsletter.gif' class='noborder' style='margin-left:4px;vertical-align:top;' title='Newsletter-Benachrichtigung' alt=''>";
        else $icn_newsletter = "";
    ...
    if($do=='vorschau' AND $modus_termin=='repeat'){
        ...
        if($vorschau['datum_end']>'' AND $vorschau['datum_end']!=='0000-00-00'){
            ...
            for($x=0;$x<=$count_termine;$x++){
                ...
                if(isset($_SESSION[$db_table]['termin_'])) $checked = (in_array($tmp_termin,$_SESSION[$db_table]['termin_']))?" checked":"";
