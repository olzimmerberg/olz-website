...
if(empty($event)) $feedback = "Variable 'event' ist leer.<br>";

// Resultatdatei
if($local) $root = "http://olzimmerberg.ch/zol/"; // Resultat von olzimmerberg-Server abholen
elseif($include_param == "karten") $root="zol/";
else $root = "" ;
...
/*    $fp = fopen($result_file, 'r');
    ...
    if(empty($file)) $feedback .= "Resultatdatei ist leer.";
    else $feedback .= "Resultatdatei gelesen: ".date("j.n.y H:i:s").", '".$result_file."'";
    ...
    foreach($rows as $row){
        ...
        elseif($row[0]>""){
                if(isset($debug)) echo str_replace(" ","***",implode(" ",$row))."<br>";
                $var = $row[0];
                $pos1 = strpos($var,"(");
                $pos2 = strpos($var,")");
